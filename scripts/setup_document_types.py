"""
Setup document types. Run from Document_Generator_backend directory:
    cd Document_Generator_backend && python scripts/setup_document_types.py

Or set BACKEND_DIR to point to the backend folder.
"""
import os
import django
import sys
...
BACKEND_DIR = os.environ.get(
    "BACKEND_DIR",
    os.path.join(os.path.dirname(__file__), "..", "Document_Generator_backend"),
)
...
def setup_types():
    ...
    template_path = os.path.join(BACKEND_DIR, 'backendapp', 'templates', 'backendapp', 'document.html')
    # If template doesn't exist at the expected path, try to find it
